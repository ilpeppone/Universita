...
int main(int argc, char** argv) {
     ...
     * la kill() a tutto il gruppo di processi (invocando kill(0)), non ne avrei bisogno. */
    ...
    for (i = 0; i < Nf; ++i) {
        ...
        if (pid[i] < 0) {
            ...
            // Impostazione della gestione del segnale SIGUSR1 con signal
            if (signal(SIGUSR1, gestore) == SIG_ERR) {
                perror("Errore nella signal, SIGUSR1 figlio");
                exit(-3);
            }
